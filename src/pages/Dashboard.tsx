...
import { getIsSameDate } from "../utils/DateFunctions";
...
export const Dashboard = () => {
        ...
        <div className="flex flex-col max-lg:flex-row justify-between align-middle items-center lg:w-72 px-5">
          <div className="flex flex-col max-lg:flex-row max-lg:justify-between justify-center items-center">
            <div className="text-4xl font-bold my-10">
              <Link to="/About">
                <img
                  src="https://avatars.githubusercontent.com/u/111740948?s=200&v=4"
                  className="h-20 sm:h-16 rounded-xl hover:brightness-125 hover:scale-105 transition duration-150 ease-in-out"
                  alt="Logo"
                />
              </Link>
            </div>
  ...
  const currentData1 = data.transactions?.filter(
    (item: any) => item.type !== "TRANSFER"
  );
  if (currentData != undefined && currentData1.length > 0) {
    currentData.current = currentData1.filter((item: any) =>
      getIsSameDate(item?.dateTime, month.toString(), year.toString())
    );
    sum.current = totalSum(currentData.current, account, category);
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full">
      ...
      <div
        id="ch2"
        className="hidden  flex-col w-full text-4xl font-bold text-center py-10 fade-in scrl"
      >
        {sum.current === 0 ? (
          <div className="text-red-500">No Transactions Found</div>
        ) : (
          <div>Total: {sum.current}</div>
        )}
        ...
        <div className="flex flex-wrap flex-row mx-auto justify-center items-center">
          {currentData?.current.map((item: any) =>
