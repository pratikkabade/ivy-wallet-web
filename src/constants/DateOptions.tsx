...
if (theDataParsed.transactions === null) {
  const data = () => {
    ...
    const uniqueTransactions = transactions
      ...
      .filter(
        (item: any) =>
          item.transactionYear !== "N/A" && item.transactionMonth !== "N/A"
      );
  ...
  const uniqueYears = data().uniqueTransactions.reduce(
    (acc: any, current: any) => {
      const x = acc.find(
        (item: any) => item.transactionYear === current.transactionYear
      );
      if (!x) {
