...
export const JSONdata = () => {
    ...
    if (url) {
      ...
      axios
        ...
        .then((res) => {
          ...
          localStorage.setItem("theData", JSON.stringify(res.data));
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  useEffect(() => {
    ...
    if (url) {
      fetchData(JSON.parse(url));
      setLocalUrl(JSON.parse(url));
    }
  }, []);
  ...
  if (data?.transactions !== undefined) {
    ...
    const accountData = data.accounts.map((account: any) => {
      const sum = data.transactions
        .filter((item: any) => item.accountId === account.id)
  ...
  const saveToLocalStorage = () => {
    if (localUrl) {
      localStorage.setItem("theURL", localUrl);
      setState({ url: localUrl });
      fetchData(localUrl);
    }
  };
      ...
      <label className="m-3 mb-5">
        <div className="flex flex-row justify-center align-middle items-center">
          ...
          <div className="block dark:text-white">
            ...
            <button className="text-sky-700 dark:text-sky-300 hover:brightness-150 hover:underline mx-2">
              ivy-wallet-web
            </button>
          ...
          {allow ? (
            <button
              ...
              type="button"
              className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-1 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 w-fit h-fit"
            >
              Save
            </button>
          ) : (
            <button
              disabled
              type="button"
              className="text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:ring-blue-100 font-medium rounded-lg text-sm px-2 py-1 dark:bg-blue-600 dark:hover:bg-blue-500 focus:outline-none dark:focus:ring-blue-600 w-fit h-fit hover:cursor-not-allowed"
            >
              Save
            </button>
          )}
